...
using TCEQ's SWQM data by TCEQ assessment units.

The `script.R` file is a long script that uses ggplot
...
of errors when generating plots.

The `functionalized_script.R` accomplishes the same task in 
...
that read and format data before plotting. 

Installation instructions for packages that are required for these scripts:
```r
